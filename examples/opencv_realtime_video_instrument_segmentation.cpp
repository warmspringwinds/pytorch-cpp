...
int main()
{
  ...
  auto net = torch::resnet34_8s_surgical_binary();

  net->load_weights("resnet34_sugical_weight.h5");
  net->cuda();
  ...
  { 
    ...
    resize(frame, frame, Size(0, 0), 0.6, 0.6, INTER_CUBIC);
    ...
    // 15 is a class for a person, it was 15 here instead of 1
    auto layer = softmaxed[1].contiguous().view({output_height, output_width, 1}).toBackend(Backend::CPU);
